...
function paginateData(data, pageSize, startIndex) {
  ...
  if (sort == "HOPE") {
    ...
    array = data.sort((a, b) => {
      const dateA = a.presentation_hope_date
        ? new Date(a.presentation_hope_date)
        : null;
      const dateB = b.presentation_hope_date
        ? new Date(b.presentation_hope_date)
        : null;
...
const showListData = async () => {
  ...
  try {
    const response = await fetch(
      `${host}/community/presentation/search`,
      requestOptions
    );
    ...
    if (code === 200) {
      ...
      $("#dataTable").on("click", "tr", function () {
        ...
        if (Array.isArray(rowData)) {
          ...
          url =
            url == "n"
              ? "/admin/presentation_register"
              : "/admin/presentation_detail";
      ...
      paginatedData.forEach(function (row, i) {
        DT.row
          .add([
            i + startIndex + 1,
            row.presentation_org_name,
            row.presentation_hope_date == null
              ? "-"
              : dayjs(row.presentation_hope_date).format("YYYY-MM-DD"),
            row.title,
